...
message_2_receipt(unsigned char *data, size_t len, uint16_t code, 
  ...
  edhoc_code = code;
  fprintf(stderr,"message_2 receipt code is %d \n", code);
  return 0;
}
...
//  fprintf(stderr,"TH_3_gen length %d \n", (int) nr);
...
edhoc_cipher2_check(uint8_t *CIPHER, uint8_t *G_Y, uint8_t *G_XY,
  ...
  fprintf(stderr,"TH_2 is :\n");
  for (uint qq = 0; qq < 32; qq++)fprintf(stderr," %02x",TH_2[qq]);
  fprintf(stderr,"\n");
   fprintf(stderr,"prk_2e is :\n");
  for (uint qq = 0; qq < 32; qq++)fprintf(stderr," %02x", prk_2e[qq]);
  fprintf(stderr,"\n"); 
  uint8_t *pt = CIPHER;
...
//    fprintf(stderr, "\n");
    ...
    if (ret != 0){
     ...
     for (uint qq =0; qq < 8; qq++)fprintf(stderr," %02x",MAC_2[qq]);
     fprintf(stderr,"\n");
...
//    fprintf(stderr,"M_2_len is %d \n", (int)M_2_len);
//    for (uint qq = 0; qq < M_2_len; qq++)fprintf(stderr," %02x", M_2[qq]);
//    fprintf(stderr,"\n");
...
edhoc_create_message_3(coap_string_t *message_1, coap_string_t *message_2, coap_string_t *message_3){
  ...
  fprintf(stderr,"TH_3 \n");
  for (uint qq =0; qq < 32; qq++)fprintf(stderr," %02x",TH_3[qq]);
  fprintf(stderr,"\n");
  ...
  for (uint qq =0; qq < 8; qq++)fprintf(stderr," %02x",MAC_3[qq]);
  fprintf(stderr,"\n");
    M_3_len = edhoc_M_x(&M_3, IR, TH_3, hash_len,
                             MAC_3, tag_len);  
   if (M_3_len < 0){
...
//   fprintf(stderr,"M_3_len %d \n",(int) M_3_len);
//  for (uint qq =0; qq < M_3_len; qq++)fprintf(stderr," %02x",M_3[qq]);
//  fprintf(stderr,"\n");
...
hnd_post_edhoc(coap_context_t  *ctx UNUSED_PARAM,
  ...
  if (edhoc_state == EDHOC_MESSAGE_1){  /* reception of message_1 */
    ...
    ok = edhoc_receive_message_1(&edhoc_message_1,
                          ...
                          &add, &add_len);
//  fprintf(stderr,"edhoc_receive_message_1 \n");
//    for (uint qq =0; qq < edhoc_message_1.length; qq++)fprintf(stderr, " %02x",edhoc_message_1.s[qq]);
//  fprintf(stderr,"\n");                       
    if (ok == 0){
      ...
      edhoc_state++;
//   fprintf(stderr,"edhoc_send_message_2 \n");
//    for (uint qq =0; qq < edhoc_message_2.length; qq++)fprintf(stderr, " %02x", edhoc_message_2.s[qq]);
//  fprintf(stderr,"\n");                       
...
//   fprintf(stderr,"edhoc_receive_message_3 \n");
//    for (uint qq =0; qq < edhoc_message_3.length; qq++)fprintf(stderr, " %02x",edhoc_message_3.s[qq]);
//  fprintf(stderr,"\n");                       
    ok = edhoc_receive_message_3(&edhoc_message_4, &edhoc_message_3, &edhoc_message_2, &edhoc_message_1);
    edhoc_state = 0;
    if (ok == 0){
//  fprintf(stderr,"edhoc_send_message_4 \n");
//    for (uint qq =0; qq < edhoc_message_4.length; qq++)fprintf(stderr, " %02x",edhoc_message_4.s[qq]);
//  fprintf(stderr,"\n");                       
      response->code = COAP_RESPONSE_CODE(204);
      coap_add_data_blocked_response(resource, session, request, response, token,
                                 COAP_MEDIATYPE_APPLICATION_EDHOC, -1,
...
//  fprintf(stderr,"edhoc_recieve_message_2 \n");
//       for (uint qq =0; qq < edhoc_message_2.length; qq++)fprintf(stderr, " %02x",edhoc_message_2.s[qq]);
//     fprintf(stderr,"\n");                       
...
//     fprintf(stderr,"edhoc_send_message_1 \n");
//       for (uint qq =0; qq < edhoc_message_1.length; qq++)fprintf(stderr, " %02x",edhoc_message_1.s[qq]);
//     fprintf(stderr,"\n");                       
		 }
         break;
...
//       fprintf(stderr,"edhoc_send_message_3 \n");
//       for (uint qq =0; qq < edhoc_message_3.length; qq++)fprintf(stderr, " %02x",edhoc_message_3.s[qq]);
//     fprintf(stderr,"\n");                              
		     break;
