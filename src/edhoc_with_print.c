...
/* certificates that contains public key and key file  */
/* Receiver keys */
#define X509_R_ES            "./certificates/brski/certs/ca-regis.crt"
#define KEY_R_ES             "./certificates/brski/private/ca-regis.key"
#define KEY_R_ed             "./certificates/brski/private/ca-regis_ed25519_key.der"
#define X509_R_ed            "./certificates/brski/certs/ca-regis_ed25519_crt.der"
/* Initiator keys  */
#define X509_I_ES            "./certificates/brski/intermediate/certs/pledge.crt"
#define X509_I_ed            "./certificates/brski/intermediate/certs/pledge_ed25519_crt.der"
#define KEY_I_ES             "./certificates/brski/intermediate/private/pledge.key"
#define KEY_I_ed             "./certificates/brski/intermediate/private/pledge_ed25519_key.der"
...
uint8_t edhoc_test = 1;   /* 0 is no test vector */
                          /* 1 is use test vector  */

/* test vector ed25519 keys */
uint8_t Pub_25519_resp[] = 
{0xdb, 0xd9, 0xdc, 0x8c, 0xd0, 0x3f, 0xb7, 0xc3, 0x91, 0x35, 0x11, 0x46, 0x2b, 0xb2, 0x38, 0x16, 0x47, 0x7c, 0x6b, 0xd8, 0xd6, 0x6e, 0xf5, 0xa1,
0xa0, 0x70, 0xac, 0x85, 0x4e, 0xd7, 0x3f, 0xd2};

uint8_t Priv_25519_resp[] =
{0xdf, 0x69, 0x27, 0x4d, 0x71, 0x32, 0x96, 0xe2, 0x46, 0x30, 0x63, 0x65, 0x37, 0x2b, 0x46, 0x83, 0xce, 0xd5, 0x38, 0x1b, 0xfc, 0xad, 0xcd, 0x44, 0x0a, 0x24, 0xc3, 0x91, 0xd2, 0xfe, 0xdb, 0x94};

uint8_t Priv_25519_init[] = 
{0x2f, 0xfc, 0xe7, 0xa0, 0xb2, 0xb8, 0x25, 0xd3, 0x97, 0xd0, 0xcb, 0x54, 0xf7, 0x46, 0xe3, 0xda, 0x3f, 0x27, 0x59, 0x6e, 0xe0, 0x6b, 0x53, 0x71, 0x48, 0x1d, 0xc0, 0xe0, 0x12, 0xbc, 0x34, 0xd7};

uint8_t Pub_25519_init[] =
{0x38, 0xe5, 0xd5, 0x45, 0x63, 0xc2, 0xb6, 0xa4, 0xba, 0x26, 0xf3, 0x01, 0x5f, 0x61, 0xbb, 0x70, 0x6e, 0x5c, 0x2e, 0xfd, 0xb5, 0x56, 0xd2, 0xe1, 0x69, 0x0b, 0x97, 0xfc, 0x3c, 0x6d, 0xe1, 0x49};

/* test vector ES256 secp256r1 keys  */

uint8_t Priv_256_init[32] = 
...
message_2_receipt(unsigned char *data, size_t len, uint16_t code) {
  if (code >> 5 == 2){
    ...
    edhoc_message_2.length = len;
    edhoc_message_2.s = coap_malloc(len);
    memcpy(edhoc_message_2.s, data, len);
  } else {
    if (edhoc_message_2.s != NULL)coap_free(edhoc_message_2.s);
...
message_4_receipt(unsigned char *data, size_t len, uint16_t code) {
  if (code >> 5 == 2){  
    ...
    edhoc_message_4.length = len;
    edhoc_message_4.s = coap_malloc(len);
    memcpy(edhoc_message_4.s, data, len);
  } else {
    if (edhoc_message_4.s != NULL)coap_free(edhoc_message_4.s);
...
//    fprintf(stderr,"edhoc_create_key, info is: \n");
//    for (uint qq =0; qq < info_len;qq++)fprintf(stderr," %02x",info[qq]);
//    fprintf(stderr,"\n");
...
edhoc_edDSA_KEY( uint8_t **KEY_F, size_t *KEY_F_len){
    char k_file_I_ed[] = KEY_I_ed;
    char k_file_R_ed[] = KEY_R_ed;
    char *key_file = NULL;
    int16_t alg = current_cipher_suite->sign_alg; 
    if (alg == COSE_Algorithm_EdDSA){
      if (edhoc_function == 'I')key_file = k_file_I_ed;
      if (edhoc_function == 'R')key_file = k_file_R_ed;
    ...
    *KEY_F_len = size;
//    fprintf(stderr,"contents of key-file with size is %d,   *KEY_F is %p \n", size, *KEY_F);
//    for (uint qq = 0; qq < size; qq++)fprintf(stderr," %02x",(*KEY_F)[qq]);
//    fprintf(stderr,"\n");
    return 0;
}
...
edhoc_ID_CRED(  char IR,
    ...
    char c_file_I_ed[] = X509_I_ed;   
    char c_file_R_ed[] = X509_R_ed;      
    char c_file_I_ES[] = X509_I_ES;   
    char c_file_R_ES[] = X509_R_ES; 
    char *cert_file    = NULL; 
    ...
    if (IR == edhoc_function){  /* use local original certificate files */
      if (alg == COSE_Algorithm_EdDSA){
        if (IR == 'I')cert_file = c_file_I_ed;
        if (IR == 'R')cert_file = c_file_R_ed;
      } else if (alg == COSE_ALGORITHM_ES256){
        if (IR == 'I')cert_file = c_file_I_ES;
        if (IR == 'R')cert_file = c_file_R_ES;     
      }
...
if ((edhoc_test == 0) || (IR == edhoc_function)){  
...
//    fprintf(stderr, "ID_CRED_x with length %d \n", nr);
//    for (uint qq = 0; qq < nr; qq++) fprintf(stderr," %02x",(*ID_CRED_x)[qq]);
//    fprintf(stderr,"\n");
...
/*    
    fprintf(stderr,"CRED_x with length %d \n", nr);
    for (uint qq = 0; qq < nr; qq++) fprintf(stderr," %02x",(*CRED_x)[qq]);
    fprintf(stderr,"\n");
    * */
...
//   fprintf(stderr, " M_x with length M_x_len %d \n",nr);
//   for (uint qq = 0 ; qq < nr; qq++)fprintf(stderr," %02x",(*M_x)[qq]);
//   fprintf(stderr,"\n");
...
edhoc_A_3ae(uint8_t **A_3ae, uint8_t *TH_3){
   ...
   *A_3ae = pt;   
   ...
   assert( nr < A_3ae_len);
 /*  
   fprintf(stderr, "A_3ae has length nr %d :\n", nr);
   for (uint qq =0 ; qq < nr; qq++) fprintf(stderr," %02x", (*A_3ae)[qq]);
   fprintf(stderr,"\n");
   * */
...
//   fprintf(stderr, "A_xm has length nr: %d \n", nr);
//   for (uint qq =0 ; qq < nr; qq++) fprintf(stderr," %02x", (*A_xm)[qq]);
//   fprintf(stderr,"\n");
//   fprintf(stderr, "end of edhoc_a_xm \n");
...
//  fprintf(stderr," TH_4 input with length %d \n", nr);
//  for (uint qq = 0; qq < nr; qq++)fprintf(stderr,"%02x",start[qq]);
//  fprintf(stderr,"\n"); 
...
//  fprintf(stderr," TH_4 with length %d \n", hash_len);
//  for (uint qq = 0; qq < hash_len; qq++) fprintf(stderr," %02x", TH_4[qq]);
//  fprintf(stderr,"\n"); 
  coap_free(start);  
  return 0;
}
...
//  fprintf(stderr," TH_3 input with length %d \n", nr);
//  for (uint qq = 0; qq < nr; qq++)fprintf(stderr,"%02x",start[qq]);
//  fprintf(stderr,"\n");
  
  /* still needs parametrization from current cipher-suite */
...
//  fprintf(stderr," TH_3 with length %d \n", COSE_ALGORITHM_SHA_256_256_LEN);
//  for (uint qq = 0; qq < COSE_ALGORITHM_SHA_256_256_LEN; qq++) fprintf(stderr," %02x", TH_3[qq]);
//  fprintf(stderr,"\n"); 
...
//  fprintf(stderr," TH_2 with length %d \n", COSE_ALGORITHM_SHA_256_256_LEN);
//  for (uint qq = 0; qq < COSE_ALGORITHM_SHA_256_256_LEN; qq++) fprintf(stderr," %02x", TH_2[qq]);
//  fprintf(stderr,"\n");
...
edhoc_gen_CIPHER3(uint8_t *TH_3, uint8_t *prk_x, uint8_t *CIPHER_3, uint8_t *in, size_t in_len){
    ...
    edhoc_create_key( &IV_3ae, nonce_len, iv_3ae, sizeof(iv_3ae) -1,
                      TH_3, prk_x);  
//    fprintf(stderr,"prk_x is ");
//    for (uint qq = 0; qq < 32; qq++)fprintf(stderr," %02x",prk_x[qq]);
//    fprintf(stderr,"\n");
...
void
dump_context(edhoc_context_t  *edhoc_ctx){
      unsigned char local[65];
      fprintf(stderr,"dump context %p \n", edhoc_ctx);
      fprintf(stderr,"dump pub key X \n");
      size_t len = mbedtls_mpi_size(&edhoc_ctx->pub->X);
    int ret = mbedtls_mpi_write_binary(&edhoc_ctx->pub->X, local, len);
    if( ret != 0 )
    {
        printf(  " internal error \n");
        return;
    }  
    for (uint qq = 0; qq < len ;qq++)fprintf(stderr," %02x",local[qq]);
    fprintf(stderr,"\ndump pub key Y \n");   
    len = mbedtls_mpi_size(&edhoc_ctx->pub->Y);
    ret = mbedtls_mpi_write_binary(&edhoc_ctx->pub->Y, local, len);
    if( ret != 0 )
    {
        printf(  " internal error \n");
        return;
    ...
    for (uint qq = 0; qq < len ;qq++)fprintf(stderr," %02x",local[qq]);   
    fprintf(stderr,"\n dump private key \n");
    len = mbedtls_mpi_size(edhoc_ctx->priv);
    ret = mbedtls_mpi_write_binary(edhoc_ctx->priv, local, len);
    if( ret != 0 )
    {
        printf(  " internal error \n");
        return;
...
fprintf(stderr,"public key G with length: %d \n", COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
for (uint qq =0 ; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++) fprintf(stderr," %02x",Pub[qq]);
fprintf(stderr,"\n\n");
...
fprintf(stderr,"private key  with length: %d \n", COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
for (uint qq =0 ; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++) fprintf(stderr," %02x",Priv[qq]);
fprintf(stderr,"\n\n");
...
fprintf(stderr,"generated private ephemeral key P with length: %d \n", COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
for (uint qq =0 ; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++) fprintf(stderr," %02x",P[qq]);
fprintf(stderr,"\n\n");
...
//fprintf(stderr,"stored public ephemeral key G with length: %d \n", COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
//for (uint qq =0 ; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++) fprintf(stderr," %02x",G[qq]);
//fprintf(stderr,"\n");
...
/* edhoc_ctx->shar contains shared key  goes to G_XY */
 ...
 fprintf(stderr,"generated shared ephemeral key G_S is with length: %d \n", (int)G_S_len);
for (uint qq =0 ; qq < G_S_len; qq++) fprintf(stderr," %02x",G_S[qq]);
fprintf(stderr,"\n\n"); 
...
edhoc_prk_shared_secret(uint8_t *G_R, uint8_t *X, uint8_t *shared){
  fprintf(stderr," used public G_XY/G_IR is  ");
  for (uint qq = 0 ; qq < 32; qq++)fprintf(stderr, " %02x",G_R[qq]);
  fprintf(stderr,"\n private XY/RI is      ");
  for (uint qq = 0 ; qq < 32; qq++)fprintf(stderr, " %02x",X[qq]);  
  fprintf(stderr,"\n");  
...
edhoc_edDSA_CERT( char IR, uint8_t **DER, size_t *DER_len){
    ...
    *DER = read_file_mem(cert_file, DER_len);
...
edhoc_ES256_KEY( mbedtls_pk_context *mbed_ctx){ 
    int16_t alg = current_cipher_suite->sign_alg;
    char k_file_I_ES[] = KEY_I_ES;
    char k_file_R_ES[] = KEY_R_ES;
    char *key_file = NULL;  
    if (alg == COSE_Algorithm_EdDSA){
      coap_log(LOG_ERR, "mbedtls cannot parse ED25519 key file \n");
      return -1;
    } else if (alg == COSE_ALGORITHM_ES256){
      if (edhoc_function == 'I')key_file = k_file_I_ES;
      if (edhoc_function == 'R')key_file = k_file_R_ES;     
    } else {
    ...
    ret = mbedtls_pk_parse_keyfile( mbed_ctx, key_file, pledge_pwd );
    if( ret != 0 )
    {
        mbedtls_strerror( ret, err_buf, sizeof(err_buf) );
...
/* test vector for ED25519 */
   if (edhoc_function == 'I'){
     *priv_key = Priv_25519_init;
   } else {
     *priv_key = Priv_25519_resp;
   }
}
 fprintf(stderr, " edhoc_EdDSA_privkey key_size is %d\n", COSE_ALGORITHM_Ed25519_PUB_KEY_LEN);
 for (uint qq = 0; qq < COSE_ALGORITHM_Ed25519_PUB_KEY_LEN; qq++)fprintf(stderr," %02x",(*priv_key)[qq]);
 fprintf(stderr,"\n");
   return 0;
}
...
if (edhoc_test == 0){
  ...
  mbedtls_ecp_group_load(&key_pair->grp, MBEDTLS_ECP_DP_SECP256R1);
  if (edhoc_function == 'I')
                ret = mbedtls_mpi_read_binary( &key_pair->d,
                Priv_256_init, sizeof(Priv_256_init));
  else ret = mbedtls_mpi_read_binary( &key_pair->d,
                Priv_256_resp, sizeof(Priv_256_resp));
  if (ret != 0)
            {
                char err_buf[CRT_BUF_SIZE];  
		            mbedtls_strerror( ret, err_buf, sizeof(err_buf));
                printf( " failed\n  !mbedtls_mpi_read_binary returned %d - %s\n", -ret, err_buf );
                return 1;
            }
...
edhoc_EdDSA_pubkey(char IR, uint8_t **pub_key){
  ...
  size_t   DER_len = 0;
  size_t   key_size = 0;
if (edhoc_test == 0){  /* use certificates */
...
} else {     /* test vector for ED25519*/
  if (edhoc_function == 'R'){
    *pub_key = Pub_25519_init;
  } else {
    *pub_key = Pub_25519_resp;
  } /* test vector for ES256 */
  key_size = 32;
} 
  fprintf(stderr, " edhoc_EdDSA_pubkey key_size is %d\n", (int)key_size);
  for (uint qq = 0; qq < key_size; qq++)fprintf(stderr," %02x",(*pub_key)[qq]);
  fprintf(stderr,"\n");
  return 0;
}
...
if (edhoc_test == 0){  /* use certificates */
  /* preparation of ecp pub key */
    ...
    mbed_ctx->pk_ctx = key_pair; 
} else {   /* test vector for ES256 */
  mbedtls_ecp_group_load(&key_pair->grp, MBEDTLS_ECP_DP_SECP256R1);
  char err_buf[CRT_BUF_SIZE];  
  if (IR == 'I'){
      ret = mbedtls_mpi_read_binary( &key_pair->Q.X,
                                           Pub_256_init_X, sizeof(Pub_256_init_X));
      if (ret == 0) ret = mbedtls_mpi_read_binary( &key_pair->Q.Y,
                                          Pub_256_init_Y, sizeof(Pub_256_init_Y));
      if (ret == 0) ret = mbedtls_mpi_lset(&key_pair->Q.Z, 1);                                    
  } else {
      ret = mbedtls_mpi_read_binary( &key_pair->Q.X,
                                           Pub_256_resp_X, sizeof(Pub_256_resp_X));
      if (ret == 0)ret = mbedtls_mpi_read_binary( &key_pair->Q.Y,
                                           Pub_256_resp_Y, sizeof(Pub_256_resp_Y));
      if (ret == 0) ret = mbedtls_mpi_lset(&key_pair->Q.Z, 1);                                    
  }
  if (ret != 0)
            {
...
edhoc_create_prk_4x3m(uint8_t *prk_4x3m, uint8_t *prk_3e2m){
  ...
  if ((edhoc_method == 2) || (edhoc_method == 3)){
    if (edhoc_function == 'R'){   /* responder : G_I -> G_RI; Y -> XY */
      ...
      fprintf(stderr,"edhoc_create_prk_4x3m, responder with G_RI, XY, with ctx is %p \n", ctx);
      ...
      fprintf(stderr,"initiator with G_XY, RI \n");
    ...
    fprintf(stderr,"prk_4x3m                      is ");
  } else {
    fprintf(stderr,"prk_4x3m copied from prk_3e2m is  ");
    memcpy(prk_4x3m, prk_3e2m, COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
  }
  for (uint qq = 0; qq < COSE_ALGORITHM_ECDH_PRIV_KEY_LEN; qq++)fprintf(stderr, " %02x",prk_4x3m[qq]);
  fprintf(stderr,"\n");
  return 0;
}
...
edhoc_create_prk_3e2m(uint8_t *prk_3e2m, uint8_t *prk_2e){
  ...
  if ((edhoc_method == 1) || (edhoc_method == 3)){
    if (edhoc_function == 'R'){   /* responder : G_X -> G_XY; R -> RI */
      ...
      fprintf(stderr,"edhoc_create_prk_3e2m, responder with G_XY, RI, ctx is %p \n", ctx);
      ret = edhoc_prk_shared_secret(G_XY, priv, shared);
      if (ret != 0) return ret;
      ...
      fprintf(stderr,"edhoc_create_prk_3e2m, initiator with G_RI, XY ctx is %p \n", ctx);
      ret = edhoc_prk_shared_secret(pub, XY, shared);
      if (ret != 0) return ret;      
    }
    hkdf_extract(prk_2e, COSE_ALGORITHM_ECDH_PUB_KEY_LEN, shared, COSE_ALGORITHM_ECDH_PUB_KEY_LEN, prk_3e2m); 
    fprintf(stderr,"prk_3e2m                     is ");
  } else {
    fprintf(stderr,"prk_3e2m copied from prk_2e  is ");
    memcpy(prk_3e2m, prk_2e, COSE_ALGORITHM_ECDH_PUB_KEY_LEN);
  }
  for (uint qq = 0; qq < COSE_ALGORITHM_ECDH_PRIV_KEY_LEN; qq++)fprintf(stderr, " %02x",prk_3e2m[qq]);
  fprintf(stderr,"\n");
  return 0;
}
...
//  fprintf(stderr,"prk_2e \n");
//  for (uint qq = 0; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++)fprintf(stderr," %02x", prk_2e[qq]);
//  fprintf(stderr,"\n");
  edhoc_data_2(&data_2, &data_2_len, G_Y);
//  fprintf(stderr,"data_2 with length %d \n", data_2_len);
//  for (uint qq =0; qq < data_2_len; qq ++) fprintf(stderr, " %02x", data_2[qq]);
//  fprintf(stderr,"\n");
...
//  fprintf(stderr,"CIPHERTEXT2 with length : %d \n",CIPHERTEXT_LEN);
//  for (uint qq = 0; qq < CIPHERTEXT_LEN; qq++) fprintf(stderr," %02x", CIPHERTEXT_2[qq]);
//    fprintf(stderr,"\n");
  if (ok < 0){
    ok = -2;
    goto exit3;
  ...
  if (CIPHERTEXT_2 == NULL)goto exit3;
    ...
    edhoc_create_key( &K_2e, CIPHERTEXT_LEN, keystream, sizeof(keystream) - 1,
                      TH_2, prk_2e);   
//    fprintf(stderr,"K_2e with length : %d \n",CIPHERTEXT_LEN);
//    for (uint qq = 0; qq < CIPHERTEXT_LEN; qq++) fprintf(stderr," %02x", K_2e[qq]);
//    fprintf(stderr,"\n");                                        
    /* construct CIPHERTEXT from CIPHERTEXT xor K_2e */
    for (uint qq = 0; qq < CIPHERTEXT_LEN; qq++) CIPHERTEXT_2[qq] = CIPHERTEXT_2[qq] ^ K_2e[qq];
    coap_free(K_2e);
//    fprintf(stderr, "reconstructed CIPHERTEXT is with length %d \n", CIPHERTEXT_LEN);
//    for (uint qq = 0; qq < CIPHERTEXT_LEN; qq++) fprintf(stderr," %02x", CIPHERTEXT_2[qq]);
//    fprintf(stderr,"\n");
 ...
 //   fprintf(stderr, "\nID_CRED_R is with length %d \n", ID_CRED_R_len);
//    for (uint qq = 0; qq < ID_CRED_R_len; qq++) fprintf(stderr," %02x", ID_CRED_R[qq]);
//    fprintf(stderr,"\n");
...
//    fprintf(stderr," MAC_2    ");
//    for (uint qq =0; qq < tag_len; qq ++) fprintf(stderr, " %02x", MAC_2[qq]);
//    fprintf(stderr,"\n");
...
edhoc_receive_message_2(coap_string_t *message_2, uint8_t **CIPHER,
  ...
  *C_R_len = 0;
  ...
  ok = cbor_get_string_array(&buf, &key, &key_len);
  if(ok != 0){
    coap_log(LOG_ERR,  "EDHOC message_2 is rrupted\n");
    return -2;
  } /* if ok */
...
edhoc_create_message_4(uint8_t *CIPHER3, size_t CIPHER3_len, 
  ...
  if (ok != 0) return -2;
  ok = edhoc_gen_MAC_4( TH_4, prk_4x3m, MAC_4);
//  fprintf(stderr,"MAC_4 is \n");
//  for (uint qq = 0; qq < tag_len; qq++) fprintf(stderr," %02x",MAC_4[qq]);
//  fprintf(stderr,"\n");
  if (ok != 0) return -2;
...
//  fprintf(stderr,"data_4 with lenght %d \n", data_4_len);
//  for (uint qq = 0; qq < data_4_len; qq++) fprintf(stderr," %02x", data_4[qq]); 
//  fprintf(stderr,"\n"); 
...
edhoc_cipher3_check(uint8_t *CIPHER3, size_t CIPHER3_LEN, coap_string_t *message_2, 
  ...
  ret = edhoc_receive_message_2(message_2, &CIPHER2, G_Y, &C_R, &C_R_len);
  size_t  data_2_len = (size_t)(CIPHER2 - data_2);
//  fprintf(stderr,"\n Received C_R with length %d\n",C_R_len);
//  for (uint qq = 0; qq < C_R_len; qq++) fprintf(stderr," %02x", C_R[qq]);
//  fprintf(stderr,"\n Received data_2 with length %d\n", data_2_len);
//  for (uint qq = 0; qq < data_2_len; qq++) fprintf(stderr," %02x", data_2[qq]);
//  fprintf(stderr, "\n Received message_1 \n");
//  for (uint qq = 0; qq < message_1->length; qq++) fprintf(stderr," %02x", message_1->s[qq]);  
//  fprintf(stderr,"\n"); 
  size_t  CIPHER2_LEN = message_2->length - data_2_len;
  ret = edhoc_TH_2_gen(TH_2, message_1, data_2, data_2_len);
  if (ret != 0){
  ...
  if (ret != 0) goto exit1;
//   fprintf(stderr,"P_3ae + empty tag;\n");
//   for (uint qq = 0; qq < CIPHER3_LEN - tag_len; qq++)fprintf(stderr,"%02x", P_3ae[qq]);  
//   fprintf(stderr,"\n");    
   uint8_t *signature = NULL;
...
//    fprintf(stderr," MAC_3    ");
//    for (uint qq =0; qq < tag_len; qq ++) fprintf(stderr, " %02x", MAC_3[qq]);
//    fprintf(stderr,"\n");
...
edhoc_create_message_3(coap_string_t *message_1, coap_string_t *message_2, coap_string_t *message_3){
  ...
  if (ok < 0) goto exit1;
//  fprintf(stderr,"received G_Y \n");
//  for (uint qq = 0; qq < COSE_ALGORITHM_ECDH_PUB_KEY_LEN; qq++) fprintf(stderr," %02x", G_Y[qq]);
//  fprintf(stderr,"\n Received C_R with length %d\n",C_R_len);
//  for (uint qq = 0; qq < C_R_len; qq++) fprintf(stderr," %02x", C_R[qq]);
//  fprintf(stderr,"\n Received data_2 with length %d\n", data_2_len);
//  for (uint qq = 0; qq < data_2_len; qq++) fprintf(stderr," %02x", data_2[qq]);
...
//  fprintf(stderr, "\n CIPHER2 \n");
//  for (uint qq = 0; qq < CIPHERTEXT_LEN; qq++) fprintf(stderr," %02x", CIPHERTEXT[qq]); 
//  fprintf(stderr,"\n"); 
...
//  fprintf(stderr,"data_3 with lenght %d \n", data_3_len);
//  for (uint qq = 0; qq < data_3_len; qq++) fprintf(stderr," %02x", data_3[qq]); 
//  fprintf(stderr,"\n"); 
...
//  fprintf(stderr,"P_3ae with length %d \n", P_3ae_len);
 // for (uint qq =0; qq < P_3ae_len; qq ++) fprintf(stderr, " %02x", P_3ae[qq]);
//  fprintf(stderr,"\n");
...
//    fprintf(stderr," CIPHERTEXT_3    \n");
//    for (uint qq =0; qq < CIPHERTEXT_3_len; qq ++) fprintf(stderr, " %02x", CIPHERTEXT_3[qq]);
//    fprintf(stderr,"\n");  
...
//   fprintf(stderr, "generate message_2 signature with length %d \n", signature_len);
...
//    fprintf(stderr, "K_2e key is with length %d \n", P_2e_len);
//    for (uint qq = 0; qq < P_2e_len; qq++) fprintf(stderr," %02x", K_2e[qq]);
//    fprintf(stderr,"\n");    
//  fprintf(stderr," \nCIPHERTEXT_2 before XOR with length %d\n", P_2e_len);
//  for (uint qq =0; qq < P_2e_len; qq ++) fprintf(stderr, " %02x", P_2e[qq]);
//  fprintf(stderr,"\n");
  CIPHERTEXT_2 = coap_malloc(P_2e_len);
  for (uint qq= 0 ; qq < P_2e_len; qq++) CIPHERTEXT_2[qq] = K_2e[qq]^P_2e[qq];
//  fprintf(stderr,"XOR'ed CIPHERTEXT_2 with length %d\n", P_2e_len);
  coap_free(K_2e);
//  for (uint qq =0; qq < P_2e_len; qq ++) fprintf(stderr, " %02x", CIPHERTEXT_2[qq]);
//  fprintf(stderr,"\n");
...
/* send message_3  */
  ...
  payload.s = coap_malloc(message_3->length);
  memcpy(payload.s, message_3->s,message_3->length);
  payload.length = message_3->length; 
  set_payload(&payload);
  return coap_start_request( COAP_MEDIATYPE_APPLICATION_EDHOC, ctx);
}

 
/* send start_message  for edhoc*/
static int16_t
send_start_message(coap_context_t *ctx, coap_string_t *message_1){
  coap_string_t payload =  {.length = 0 , .s = NULL};
  payload.s = coap_malloc(message_1->length);
  memcpy(payload.s, message_1->s, message_1->length);
  payload.length = message_1->length; 
  set_payload(&payload);
...
edhoc_oscore_session(coap_context_t *ctx, coap_session_t *session, 
                         edhoc_state_t *client_state, uint16_t method_corr, uint8_t suite){
     int8_t ok = 0;
     switch (*client_state) {
       case EDHOC_MESSAGE_1:
         ok = edhoc_create_message_1(&edhoc_message_1, &G_X_string, method_corr, suite);
         if (ok == 0) ok = send_start_message( ctx, &edhoc_message_1);
         break;
