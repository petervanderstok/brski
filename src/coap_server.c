           ...
           size_t *size)
{
  ...
  /* NOT BLOCK1 */  
    ...
    /* Not a BLOCK1 and no data */
       coap_error_return(COAP_RESPONSE_CODE(400), 
...
usage( const char *program, const char *version) {
  ...
  fprintf( stderr, "%s v%s -- a small CoAP implementation\n"
...
#ifdef WITH_OSCORE
     "\t-c keyfile\tfile containing key file for EDHOC verification\n"
     ...
     "\t       \t\tgiven this CA Certificate during the (D)TLS/EDHOC set up.\n"
     ...
     "\t       \t\tvalidation\n"
#ifdef WITH_OSCORE
     "\t-C certificate \tPEM file containing CA certificate for EDHOC verification\n"
     "\t        \t\tDER for ED25519, PEM for ES256\n"
#endif
...
main(int argc, char **argv) {
  ...
  /* Receiver (server) files */
    if ((cert_file != NULL) && (ca_file   != NULL)){
