...
#ifdef WITH_JOIN_PROXY
#include "JP_server.h"
#endif /* WITH_JOIN_PROXY */
...
coap_send_pdu(coap_session_t *session, coap_pdu_t *pdu, coap_queue_t *node) {
  ...
  if (session->oscore_encryption){
	/* osc_pdu has an ephemeral existence next to pdu  */
     coap_pdu_t *osc_pdu = oscore_message_encrypt( pdu, session);
     if ( osc_pdu == NULL){
        coap_log(LOG_WARNING, "OSCORE PDU could not be encrypted\n");
        return -1;
     }
	 bytes_written = coap_socket_send_pdu(
                                  sock, session, osc_pdu);                                                       
  ...
  *        must not be dropped. */
  ...
  if (session->oscore_encryption){
	/* osc_pdu has an ephemeral existence next to pdu  */
	 ...
	 bytes_written = coap_session_send_pdu(session, osc_pdu);
     coap_delete_pdu(osc_pdu);
  }
  else
#endif /* WITH_OSCORE */  
...
#ifdef WITH_JOIN_PROXY
  if (jp_is_proxy(session)){
	 result = jp_transfer(session, data, data_len);
  } else if (session->proto == COAP_PROTO_DTLS) {
#else /* WITH_JOIN_PROXY */  
...
coap_handle_dgram(coap_context_t *ctx, coap_session_t *session,
  ...
  if (opt){
    ...
    decrypt_pdu = oscore_message_decrypt(pdu, session);
    if ( decrypt_pdu == NULL){
      coap_log(LOG_WARNING, "OSCORE PDU could not be decrypted\n");
      goto error;
    }
    /* decrypt_pdu is used for dispatch  */
    coap_dispatch(ctx, session, decrypt_pdu);
...
coap_join_mcast_group(coap_context_t *ctx, const char *group_name) {
  ...
  struct addrinfo   *reslocal = NULL, *resmulti = NULL, hints, *ainfo, *found = NULL;
  int result = -1;
...
*/
  ...
  for (ainfo = resmulti; ainfo != NULL; ainfo = ainfo->ai_next) {
    if (ainfo->ai_family == AF_INET6) {
        m6req.ipv6mr_multiaddr =
                ((struct sockaddr_in6 *)ainfo->ai_addr)->sin6_addr;
        m6req.ipv6mr_interface =
                ((struct sockaddr_in6 *)ainfo->ai_addr)->sin6_scope_id;         
      found = ainfo;
      break;
    } else if (ainfo->ai_family == AF_INET) {
         m4req.imr_multiaddr.s_addr = inet_addr(group_name);
    ...
    if (endpoint->proto == COAP_PROTO_UDP 
	  ...
	  if (found->ai_family == AF_INET6) {	
          result = setsockopt(endpoint->sock.fd, IPPROTO_IPV6, IPV6_JOIN_GROUP,
                          (char *)&m6req, sizeof(m6req));
      } else if (found->ai_family == AF_INET) {  
		  result = setsockopt(endpoint->sock.fd, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                          (char *)&m4req, sizeof(m4req));
	  }
