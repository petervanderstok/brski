...
void set_scope_id(coap_address_t *connect_addr){
	if (connect_addr->addr.sa.sa_family == AF_INET6){
		if ((connect_addr->addr.sin6.sin6_addr.s6_addr[0] == 0xfe)  &&
		    (connect_addr->addr.sin6.sin6_addr.s6_addr[1] == 0x80)){  /* link_local address  */
			connect_addr->addr.sin6.sin6_scope_id = if_nametoindex("wlan0");
		}
	}
}
...
coap_network_send(coap_socket_t *sock, const coap_session_t *session, const uint8_t *data, size_t datalen) {
  ...
  if (!coap_debug_send_packet()) {
    ...
    if (!coap_address_isany(&session->addr_info.local) &&
        !coap_is_ (&session->addr_info.local))
