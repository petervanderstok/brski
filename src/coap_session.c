...
ssize_t coap_session_send(coap_session_t *session, const uint8_t *data, size_t datalen) {
  ...
  if (jp_is_proxy(session)){
	  bytes_written = jp_return_transfer( session, data, datalen);
  } else  bytes_written = coap_socket_send(sock, session, data, datalen);
#else
...
coap_tid_t coap_session_send_ping(coap_session_t *session) {
  ...
  fprintf(stderr,"coap_session_send_ping  session->state %d, COAP_SESSION_STATE_ESTABLISHED %d\n", 
         (int)session->state, COAP_SESSION_STATE_ESTABLISHED);
  ...
  fprintf(stderr,"coap_session_send_ping  ping is %p \n", (void *)ping);
#if !COAP_DISABLE_TCP
  else {
...
void coap_session_connected(coap_session_t *session) {
 ...
 /* write here extension to fill payload  */
 #ifdef WITH_BRSKI
    if (session->payload_code != NULL){
		int8_t ok = session->payload_code(q->pdu);
		session->payload_code = NULL;
	} 
#endif /* WITH_BRSKI */         
...
#ifdef WITH_JOIN_PROXY
  if (jp_not_registrar(endpoint->sock.fd)){
	  /* skip this test in registrar server, the payload is enveloped; test is done in join_proxy */
#endif /* WITH JOIN_PROXY  */
