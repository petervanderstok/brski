...
/* group-client -- simple CoAP group oscore client
 *
...
/* public key of Rikard's server  */
/*
static unsigned char 
    public_key_CR[COSE_ALGORITHM_Ed25519_PUB_KEY_LEN] =
{0x26, 0x68, 0xBA, 0x6C, 0xA3, 0x02, 0xF1, 0x4E, 0x95, 0x22, 0x28, 0xDA, 0x12, 0x50, 0xA8, 0x90, 0xC1, 0x43, 0xFD, 0xBA, 0x4D, 0xAE, 0xD2, 0x72, 0x46, 0x18,  0x8B, 0x9E, 0x42, 0xC9, 0x4B, 0x6D};
*/

/* public and private keys of group_server */
static unsigned char 
...
static uint8_t master_secret[16] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};

static uint8_t salt[8] = {0x1f, 0x2e, 0x3d, 0x4c, 0x5b, 0x6a, 0x70, 0x81}; 
static uint8_t sender_id[1] = { 0x0a };      /* client */
static uint8_t receiver_id[1] = { 0x51 };    /* server */
...
/* prepare one sender receiver context   */
  osc_ctx = oscore_derive_ctx(master_secret, 
      16, salt, 8,
      10, sender_id, 1, receiver_id, 1, NULL, 0, 
      OSCORE_DEFAULT_REPLAY_WINDOW);
  if(!osc_ctx ){
	coap_log(LOG_CRIT, "Could not create 2nd OSCORE Security Context!\n");
  }
...
main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "NrUa:b:c:e:f:h:k:m:p:s:t:o:v:A:B:C:O:P:R:E:T:u:l:K:")) != -1) {
#else
...
/* osc_ctx  points to standard oscore context  */  
  osc_ctx = oscore_set_contexts();
/* osc_ctx  points to group oscore context  */
  ...
  uint8_t *counter_signature_parameters =
      oscore_cs_key_params(COSE_Elliptic_Curve_Ed25519, 
         COSE_KTY_OKP, &counter_signature_parameters_len);
