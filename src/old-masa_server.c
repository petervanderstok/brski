                   ...
                   coap_string_t *signed_voucher)
{
  ...
  if (ct == COAP_MEDIATYPE_APPLICATION_VOUCHER_COSE_CBOR){
      ...
      voucher_request = brski_verify_cms_signature(signed_voucher_request, file_name);
...
//      for (uint qq = 0; qq < voucher_request->length; qq++)fprintf(stderr,"%c",voucher_request->s[qq]);
 //     fprintf(stderr,"\n");
  ...
  if (ct == COAP_MEDIATYPE_APPLICATION_VOUCHER_COSE_CBOR)
     ...
     ok = brski_cms_sign_payload(signed_voucher, &voucher, key_file);
  if (voucher.s != NULL)free(voucher.s);
  if (ok != 0){
...
masa_server( void )
{
    ...
    fflush( stdout );
    ret = mbedtls_x509_crt_parse_file( &srvcert, server_file_name );
    if( ret != 0 )
    {
     ...
     */
    if( ( ret = mbedtls_net_bind( &listen_fd, NULL, port_str, MBEDTLS_NET_PROTO_TCP ) ) != 0 )
    {
