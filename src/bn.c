...
void bignum_from_array(struct bn* n, uint8_t *arr, int nbytes)
{ 
  require(n, "n is null");
  require(arr, "str is null");
  require(nbytes > 0, "nbytes must be positive");
  
  /* initialize bn to zero */
  for (int qq = 0; qq < BN_ARRAY_SIZE; qq++)n->array[qq] = 0;

  int j = 0;                                  /* index into array - reading "LSB" first */
  int i = nbytes - ((j+1) * WORD_SIZE);       /* index into string array. */

  /* copy from least significant to most significant */
  while ((j  <  BN_ARRAY_SIZE + 1) && (i > -1))
  {
	DTYPE val = 0;
	int k = WORD_SIZE;
    while ( k > 0){
	...
	n->array[j] = val;
	j++;
	i = nbytes -((j+1) * WORD_SIZE);               /* step one element back in the array. */
  }
}

void bignum_to_array(struct bn* n, uint8_t *arr, int maxsize)
{ 
  require(n, "n is null");
  require(arr, "str is null");
  require(maxsize > 0, "nbytes must be positive");
  
  /* initialize to zero */
  for (int qq = 0; qq < maxsize; qq++) arr[qq] = 0;

  int j = 0;                    /* index into array - reading "LSB" first */
  int i = maxsize-1;             /* index into string array. */

  /* copy from least significant to most significant */
  while ((j  <  BN_ARRAY_SIZE) && (i > 0))
  {
	DTYPE val = n->array[j];
	int k = WORD_SIZE;
    while ( (k > 0) && (i > -1)){
    ...
    j++;               /* step one element back in the array. */
  }
}
...
void bignum_to_string(struct bn* n, char* str, int nbytes)
{
  ...
  int j = (nbytes/(2*WORD_SIZE)) - 1;        /* index into array - reading "LSB" first -> big-endian */
  int i = 0;                           /* index into string representation. */
  
  /* initialize string  to zeroes */
  for (int qq = 0 ; qq < nbytes; qq++) str[qq] = '0';
  /* reading last array-element "MSB" first -> big endian */
  while ((j > -1) && (i < nbytes + 1 - (2*WORD_SIZE)))
  {
...
// It returns res = (x^n) % p. 
void
bignum_powermod( struct bn *x, struct bn *n, struct bn *p, struct bn *res) 
{ 
	struct bn zero, one, tmp, ni, xi;
	bignum_init(&tmp);
	bignum_init(&zero);
	bignum_init(&xi);         /*inv over i: xi = x^(2i)  */
	bignum_init(&ni);
	bignum_from_int(&zero, (DTYPE_TMP)0);
	bignum_init(&one);
	bignum_from_int(&one, (DTYPE_TMP)1);
	bignum_assign(&ni, n);  /* inv over i: ni = n >> i */
	bignum_assign(&xi, x);
		
    bignum_mod(&xi, p, &tmp); /*  xi = xi % p */
    bignum_assign(res, &one);   /* res = x^0 = 1 */
    while (bignum_cmp(&ni, &zero) == LARGER){  /* while (ni > 0) */
        // If ni is odd, multiply xi with res to res 
        bignum_and(&ni, &one, &tmp); 
        if (bignum_cmp(&tmp, &one) == EQUAL){ 
           bignum_mul(res, &xi, &tmp);
           bignum_mod(&tmp, p, res);     /* res = (xi * res) mod p */
        }
        bignum_rshift(&ni, &tmp, 1); /* ni = ni/2  */
        bignum_assign(&ni, &tmp);
        bignum_mul(&xi, &xi, &tmp);   
        bignum_mod(&tmp, p, &xi);  /* xi = (xi * xi) % p;  */
    } 
    /* res contains result */ 
}

void
bignum_squaremod(struct bn *y, struct bn *p, struct bn *x){
	// if solution exists: returns square root of y under modulo p in x
	// (x*x) mod p = y
	// if solution does not exist, return x = 0;
    // Assumption: p is of the form 3*i + 4 where i >= 1  
    struct bn four, res, one, tmp;
    bignum_init(&four);
    bignum_init(&res);
    bignum_init(&one);
    bignum_init(&tmp);
    bignum_from_int(&one, (DTYPE_TMP)1);
    bignum_from_int(&four, (DTYPE_TMP)4);
    /* test that p % 4 = 3  */
    bignum_mod(p, &four, &res);
    int t = bignum_to_int(&res);
    if (t != 3) { 
        bignum_from_int(x, (DTYPE_TMP)0); /* return 0 */
        return; 
    } 

    // Try "(y^((p + 1)/4))" 
    bignum_mod(y, p, &tmp); 
    bignum_assign(y, &tmp);         /* y = y mod p */
    bignum_add(p, &one, &res);
    bignum_div(&res, &four, &tmp);
    bignum_assign(&res, &tmp);       /* res = (p+1)/4 */
/*   try: int x = power(y, (p + 1) / 4, p); */
    bignum_powermod(y, &res, p, x);
    
/*    test if ((x * x) % p == y)   */
    bignum_mul( x, x, &res);
    bignum_mod( &res, p, &tmp);
    bignum_assign(&res, &tmp);

    if (bignum_cmp(&res, y) == EQUAL)return ;          /* x contains value  */
    bignum_from_int(x, (DTYPE_TMP) 0);                 /* else x contains zero */
} 
