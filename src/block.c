...
coap_write_block_opt(coap_block_t *block, uint16_t type,
  ...
  if (data_length <= start) {
    coap_log(LOG_DEBUG, "illegal block requested data_length: %d     block->num: %d\n", (int)data_length, (int)block->num);
    return -2;
  }
                       ...
                       const uint8_t* data)
{
  ...
  block2.szx = 6;  /* default block2 size */
  coap_block_t block1 = { 0, 0, 0};
  int block2_present = 0;
  int block1_present = 0;
  ...
  if (request) {
	if (coap_get_block(request, COAP_OPTION_BLOCK1, &block1)){
		block1_present = 1;
		if (block1.m == 1){
			coap_log(LOG_WARNING," not last block1 \n");
		}
	}
    if (coap_get_block(request, COAP_OPTION_BLOCK2, &block2)) {
      block2_present = 1;
      if (length <= (block2.num << (block2.szx + 4))) {
        coap_log(LOG_WARNING, "Illegal block requested (%d > last = %zu)\n",
                 block2.num,
                 length >> (block2.szx + 4));
  ...
  if (block1_present) response->code = COAP_RESPONSE_CODE(204);
  else response->code = COAP_RESPONSE_CODE(205);
  ...
  /*
   * BLOCK2 not present in request
   */
  if (!block1_present){
    if (coap_add_data(response, length, data)) return;
  }
/* at this point:
 * or last block1 is present and block2 suggested
 * or data did not fit and block2 required
 */
    /* set initial block size, will be lowered by
     * coap_write_block_opt automatically */
    block2.num = 0;
 
    block2.szx = block1.szx;
    if (length > 0){ /* there are data to send */
      int res;
      res = coap_write_block_opt(&block2, COAP_OPTION_BLOCK2, response,
                               length);
    ...
    if (block1_present){ /* ack last block1 in response */
